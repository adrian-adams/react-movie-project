...
export const updateSearchCount = async (searchTerm, movie) => {
    ...
    try {
        ...
        if (result.documents.length > 0) {
            ...
            await database.createDocument(DATABASE_ID, COLLECTION_ID, ID.unique(), {searchTerm, count: 1, movie_id: movie.id, poster_url: `https://images.tmdb.org/t/p/w500${movie.poster_path}`})
        }
    } catch (error) {
        console.error(error);
    } 
